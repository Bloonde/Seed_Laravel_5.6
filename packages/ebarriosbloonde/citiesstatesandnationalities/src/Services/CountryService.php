...
use EBarriosBloonde\CitiesStatesAndNationalities\Configurations\CountryConfigurationImpl;
use EBarriosBloonde\CitiesStatesAndNationalities\Http\Resources\CountryResource;
use EBarriosBloonde\CitiesStatesAndNationalities\Http\Resources\CountryResourceCollection;
use EBarriosBloonde\CitiesStatesAndNationalities\Repositories\CountryRepository;
...
class CountryService extends AbstractService
{
    ...
    public function __construct()
    {
        ...
        $this->configuration_impl = new CountryConfigurationImpl();
        $this->model = $this->configuration_impl->getModelName();
        $this->repository = CountryRepository::getInstance(); //Remove  params if exist a specific repository for this resource
    }
    ...
    public function get($id, $resource = true){
        ...
        if($resource) {
            return new CountryResource($model, $this->configuration_impl);
        } else {
            return $model;
        }
    }
    ...
    public function list($input, $resource = true)
    {
        ...
        if($resource) {
            $response = new CountryResourceCollection($response);
        }
        return $response;
    }
    ...
    public static function getInstance(){
        if(self::$__instance === null){
            self::$__instance = new CountryService();
        }
